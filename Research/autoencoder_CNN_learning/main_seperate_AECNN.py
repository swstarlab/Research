    ...
    for epoch in range(1, EPOCH+1):
        ...
        for step, (data, target) in enumerate(train_loader):
            ...
            if AE_Loss >= threshold: #물어보기
                ...
                corret_ratio = crr_cnt_correct / (crr_cnt_correct + crr_cnt_wrong)
                correct_ratio_List.append(corret_ratio)
            ...
            if save_file == False and save_csv == False:
                if step % 200 == 0:
                    print('Train Epoch: {} [{}/{} ({:.0f}%)]\tAE_Loss: {:.6f}\tthreshold: {:.6f}\tAsk_rate: {:.6f}'.format(
                        epoch, step * len(data), len(train_loader.dataset),
                               100. * step / len(train_loader), AE_loss.item(), threshold, crr_ask_rate))
