...
if __name__=='__main__':
    ...
    for epoch in tqdm(range(1, EPOCH+1)):
        ...
        for step, (data, target) in enumerate(train_loader):
            ...
            CNN_loss = F.cross_entropy(x_output, target)  # 모의구동시에는 target, 실사용시에는 label
            ...
            elif CNN_loss < threshold:#안 물어보고 스스로 추론하기
                # _view_data = x.view(-1, 28 * 28)
                # _view_data = _view_data.type(torch.FloatTensor) / 255.
                # test_x = _view_data.to(DEVICE)
                # _, decoded_data = autoencoder(test_x)
            ...
            if step % 200 == 0:
                print('Train Epoch: {} [{}/{} ({:.0f}%)]\tCNN_Loss: {:.6f}\tAsk_rate: {:.6f}\tthreshold: {:.6f}'.format(
                    epoch, step * len(data), len(train_loader.dataset),
                           100. * step / len(train_loader), CNN_loss, crr_ask_rate, threshold))
